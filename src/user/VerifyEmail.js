...
const VerifyEmail = () => {
  // Get verification code passed in via URL
  const { verificationCode } = useParams();
  let navigate = useNavigate(); // useNavigate hook which helps us to re-direct to the respective route

  // Define "onFormSubmit" to log the values and display toast message that email verification code is present in the console
  const onFormSubmit = async (values) => {
    // Call the verifyEmail API and pass the field value which has verficcationCode
  ...
  // Yup validation schema for the verification code input field
  const emailVerificationSchema = Yup.object().shape({
    verificationCode: Yup.string().required(
      "Email verifcation code is required"
    ),
  });

  return (
    <div class="flex items-center justify-center min-h-screen p-5 min-w-screen">
      <div class="max-w-xl p-8 text-center text-cColor-50 bg-white shadow-xl lg:max-w-3xl rounded-3xl lg:p-12">
        <h3 class="text-2xl">Thanks for signing up </h3>
        <div class="flex justify-center">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="w-24 h-24 text-cColor-50"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
